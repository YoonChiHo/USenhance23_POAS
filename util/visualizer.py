...
def save_images(webpage, visuals, image_path, aspect_ratio=1.0, width=256, use_wandb=False, loaded_data=[0.5, 0.5]):
    ...
    for label, im_data in visuals.items():
        im = util.tensor2im(im_data, loaded_data)
...
class Visualizer():
    ...
    def __init__(self, opt):
        ...
        if opt.data_norm == 'ab_seperate':
            ...
            self.loaded_data = {"TrainA": [0.5, 0.5], "TrainB": [0.5, 0.5]}
        ...
        if self.display_id > 0:  # show images in the browser using visdom
            ...
            if ncols > 0:        # show all the images in one visdom panel
                ...
                for label, image in visuals.items():
                    image_numpy = util.tensor2im(image, self.loaded_data['TrainB'])
